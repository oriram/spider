...
def read_data_from_json_files(paths: List[str], upsample_rates: List = None, num_examples: List = None) -> List:
    results = []
    if upsample_rates is None:
        upsample_rates = [1] * len(paths)
    if num_examples is None:
        num_examples = [-1] * len(paths)
    ...
    for i, path in enumerate(paths):
        with open(path, "r", encoding="utf-8") as f:
            logger.info("Reading file %s" % path)
            data = json.load(f)
            dataset_num_examples = int(num_examples[i])
            if 0 < dataset_num_examples < len(data):
                data = list(np.random.choice(data, dataset_num_examples, replace=False))
...
class Tensorizer(object):
    ...
    def text_to_tensor(
        self, text, title = None, add_special_tokens = True
    ):
