...
from transformers import BertModel, BertTokenizer, RobertaTokenizer, AutoConfig, DPRContextEncoder

from dpr.utils.data_utils import Tensorizer
from .biencoder import BiEncoder, SharedBiEncoder
...
def get_bert_biencoder_components(args, inference_only: bool = False, **kwargs):
    ...
    if not args.weight_sharing:
        ...
        fix_question_encoder = (
            args.fix_question_encoder if hasattr(args, "fix_question_encoder") else False
        )
        biencoder = BiEncoder(
            question_encoder, ctx_encoder,
            fix_ctx_encoder=fix_ctx_encoder,
            fix_q_encoder=fix_question_encoder,
            return_pooled_ctx_repr=return_pooled_ctx_repr
        )
    else:
        biencoder = SharedBiEncoder(
            ctx_encoder, fix_ctx_encoder, return_pooled_ctx_repr
        )
...
def get_bert_tokenizer(pretrained_cfg_name: str, do_lower_case: bool = True):
    if pretrained_cfg_name:
        return BertTokenizer.from_pretrained(
            pretrained_cfg_name, do_lower_case=do_lower_case
        )
    else:
        return BertTokenizer.from_pretrained("bert-base-uncased" if do_lower_case else "bert-base-cased",
...
class HFBertEncoder(BertModel):
    ...
    def init_encoder(
        ...
        if cfg_name:
            if hasattr(cfg, "architectures") and "DPR" in cfg.architectures[0]:
                if cfg.architectures[0] == "DPRContextEncoder":
                    from transformers import DPRContextEncoder
                    encoder = DPRContextEncoder.from_pretrained(cfg_name).ctx_encoder
                else:
                    from transformers import DPRQuestionEncoder
                    encoder = DPRQuestionEncoder.from_pretrained(cfg_name).question_encoder
                to_return = cls(config=cfg, project_dim=projection_dim)
                to_return.embeddings = encoder.bert_model.embeddings
                to_return.encoder = encoder.bert_model.encoder
                to_return.pooler = encoder.bert_model.pooler
                return to_return

            return cls.from_pretrained(cfg_name, config=cfg, project_dim=projection_dim, **kwargs)
        else:
            return cls(config=cfg, project_dim=projection_dim)
    ...
    def forward(
        ...
        if isinstance(res, dict):
            ...
            hidden_states = res["hidden_states"] if "hidden_states" in res else None
        else:
            # Transformers 3
            assert isinstance(res, tuple)
            sequence_output, _, *rest = res
            hidden_states = rest[0] if rest else None
...
class BertTensorizer(Tensorizer):
    ...
    def text_to_tensor(
        self, text, title=None, add_special_tokens=True, max_length=None
    ):
        if isinstance(text, str):
            text = text.strip()

        seq_len = self.max_length if max_length is None else max_length
